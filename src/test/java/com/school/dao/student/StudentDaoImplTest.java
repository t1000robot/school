...
import com.school.dao.TestConnectionFactory;
...
class StudentDaoImplTest {
    ...
    void findStudentsByCourse_shouldReturnProperStudentNameFormat_whenCourseValueIsCorrect() {
        List<String> studentList = new StudentDaoImpl()
            .findStudentsByCourse(ApplicationMenu.dataGenerator.getCourses().get(random.nextInt(10)));
    ...
    void findStudentsByCourse_shouldReturnProperResult_withRandomCourse() {
        ...
        Course course = ApplicationMenu.dataGenerator
            .getCourses()
            .stream()
        ...
        for (Student student : ApplicationMenu.dataGenerator.getStudents()) {
    ...
    void delete_shouldDeleteStudentFromDataBase_ifInputDataAreValid() {
        Student student = ApplicationMenu.dataGenerator.getStudents().stream().findFirst().get();
    ...
    void addStudentToCourse_shouldAddStudentToCourse_whenInputDataAreValid() {
        Student actualStudent = ApplicationMenu.dataGenerator.getStudents().stream().findFirst().get();
        Course actualCourse = ApplicationMenu.dataGenerator.getCourses().stream().findFirst().get();
    ...
    void removeStudentFromCourse() {
        ...
        int testStudentId = studentsCoursesTestMap
            ...
            .findFirst()
            .get();
        Course testCourse = ApplicationMenu.dataGenerator
            .getCourses()
            .stream()
            .filter(course -> course.getId() == testCourseId)
            .findFirst()
            .get();
        Student testStudent = ApplicationMenu.dataGenerator
            .getStudents()
            .stream()
